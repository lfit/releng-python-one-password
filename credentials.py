...
def credential_summary(list, no_tags):
	...
	for credential in list:
		id = credential['id']
		# Tags might take up screen space; only show if not suppressed in output
		if no_tags:
			log.info("%s	%s", credential['id'], credential['title'])
		else:
			# Not all credentials have tags
			try:
...
def filter_credential_db(select, reject, ignore_case):
	...
	if yes_or_no("Update working credential set to selection?"):
		save_json_file(filtered_credentials, gl_credentials_db)
	else:

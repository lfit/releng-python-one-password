...
def match_elements(ignore_case, search_patterns, dictionary_list):
	...
	for search_pattern in search_patterns:
		...
		log.info(
			"Elements selected: [%s] %s", pattern_matches, search_pattern)
	return matched
...
def filter_credential_db(select, reject, ignore_case):
	...
	# Create a list to hold filtered credentials
	filtered_credentials = []
	# Get the total credentials in the current working set
	working_set = len(gl_credentials)
	
	# Create lists to hold included/excluded credentials
	selected = match_elements(ignore_case, select, gl_credentials)
	log.info("Total selected: %s/%s", len(selected), working_set)

	rejected = match_elements(ignore_case, reject, gl_credentials)
	log.info("Total selected: %s/%s", len(rejected), working_set)

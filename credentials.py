...
def tags_add(
	debug: bool = typer.Option(False, "--debug", "-d", show_default=False,
		help="Enable verbose debug output/logging"),
	overwrite: bool = typer.Option(False, "--overwrite", "-o", show_default=False,
		help="Overwrite any pre-existing tags (they are otherwise preserved)"),
	tags: list[str] = typer.Argument(None, show_default=False, help='Add tag(s) to the currently selected credentials')
	):
	...
	if len(tags) == 0:
		...
		sys.exit(1)
	credentials = caching.load_cache('credentials')
	tags_add(credentials, tags, overwrite)
...
def tags_allocate(
	debug: bool = typer.Option(False, "--debug", "-d", show_default=False,
		help="Enable verbose debug output/logging"),
	select: Optional[list[str]] = typer.Option(
		None, "--tag", "-t", help='Add tag(s) to the currently selected credentials')
	):
...
def tags_add(credentials, tags, overwrite):
	if overwrite:
		log.warning("Warning: addition operation will overwrite existing tags")
	log.info("Tags to apply: %s", tags)
	
	# Offer to show current credentials and tags
	...
	for credential in credentials:
		try:
			...
			current_tags = []
		log.info("Current tags: %s", current_tags)
		if overwrite:
			log.info("New tags: %s", tags)
		else:
			log.info("New tags: %s", (current_tags + tags))

	# Offer to show revised credentials and tags
	
	# Call command-line tool to update credentials using multiprocessing
